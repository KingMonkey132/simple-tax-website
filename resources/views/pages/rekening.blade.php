...
@section('container')
        ...
        <div class="search-feature">
            <form action="{{route('cari-data-rekening')}}" method="get" class="search">
                <label for="search">Cari: </label><input type="text" name="search" class="input-search" placeholder="Ketik Nama Rekening">
                <input type="submit" value="Cari">
            </form>
            <form action="{{route('rekening')}}" method="GET" class="refresh">
                ...
                <tr id="rekening-result">
                    ...
                    <td>
                        <div class="action-container">
                            <form action="{{route('edit-data-rekening', $item->kode_rekening)}}" method="get">
                                @csrf
                                <button type="submit">Edit</button>
                            </form>
                            <form action="{{ route('hapus-data-rekening', $item->kode_rekening) }}" method="post" id="delete-{{$item->kode_rekening}}">
                                @csrf
                                @method('delete')
